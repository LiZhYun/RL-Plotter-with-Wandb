...
def plot(df, key='eval_average_episode_rewards'):
    ...
    for idx, scenario in enumerate(main_envs):
        ...
        if idx == 0:
            hue_order = ['BPPO_SEQUENTIAL', 'BPPO_REVERSE', 'BPPO_RANDOM']
    ...
    plt.savefig(f'{save_path}/MuJoCo_aaai_feedback2.pdf')
    plt.show()

df = [pd.read_csv(f'{data_path}/{env}_aaai_feedback2.csv') for env in main_envs]
plot(pd.concat(df))